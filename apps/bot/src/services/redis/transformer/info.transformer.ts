...
const fromRedis = (value: string): IInfo => {
  ...
  return {
    workerPower: {
      ...
      [IDLE_FARM_WORKER_TYPE.spookyworker]: transformWorkerPower(
        parsed?.workerPower?.spookyworker
      )
    },
    ...
    leaderboard: {
      ...
      [IDLE_FARM_WORKER_TYPE.spookyworker]: parsed?.leaderboard?.spookyworker ?? []
    }
  };
};
