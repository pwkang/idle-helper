...
export const _memberTracker = async ({guildRole, server, client, channelId, author}: IMemberTracker) => {
  ...
  const getMessagePayload = (): BaseMessageOptions => {
    if (!guild) return {
      content: `There is no guild with role ${messageFormatter.role(guildRole.id)} setup in this server`,
    };
    return {
...
const getMemberRoleStatusMessageOptions = (
  ...
  const membersWithRole = cachedServerMembers.filter(member =>
    embedUsernames.includes(member.user.username) ||
    embedUsersId.includes(member.user.id),
  );

  const nonCachedMembersUsername = embedUsernames.filter(username => !cachedServerMembers.some(member => member.user.username === username));
  const nonCachedMembersId = embedUsersId.filter(id => !cachedServerMembers.some(member => member.user.id === id));

  const nonMembersWithRole = cachedServerMembers.filter(member =>
    !embedUsernames.includes(member.user.username) &&
    !embedUsersId.includes(member.user.id),
  );
  ...
  for (let i = 0; i < membersWithRole.length; i += 30) {
    embed.addFields({
      ...
      value: membersWithRole.slice(i, i + 30).length ? membersWithRole.slice(i, i + 30).map(member => messageFormatter.user(member.user.id)).join('\n') : 'None',
      inline: true,
    });
  }
  ...
  for (let i = 0; i < nonMembersWithRole.length; i += 30) {
    embed.addFields({
      ...
      value: nonMembersWithRole.slice(i, i + 30).length ? nonMembersWithRole.slice(i, i + 30).map(member => messageFormatter.user(member.user.id)).join('\n') : 'None',
      inline: true,
    });
  }

  let content = '';
  if (nonMembersWithRole.length || nonCachedMembersId.length || nonCachedMembersUsername.length) {
    content = [
      ...
      `\`\`\`${nonCachedMembersId.map(messageFormatter.user)} ${nonCachedMembersUsername.map(username => `@${username}`)}\`\`\``,
    ].join('\n');
  }
